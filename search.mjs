import {moves,play,pass,score,locate} from './engine.mjs';
...
list.sort((a,b)=>b.order-a.order);let keep=Number.isFinite(width)?Math.max(1,width):list.length;stats.beamPruned+=Math.max(0,list.length-keep);return list.slice(0,keep);}
const moveId=m=>m?m.l+':'+m.t+':'+m.i:'';
export function search(s,{budget=1600,width=8,maxDepth=5,pruning=true,transpositions=true}={},progress=()=>{}){
 ...
 function minimax(q,d,alpha,beta){check();stats.nodes++;if(!d||q.ended)return evaluate(q,perspective);
  ...
  for(let i=0;i<list.length;i++){check();const m=list[i];stats.expanded++;const child=play(q,m.l,m.i,m.t),val=minimax(child,d-1,alpha,beta);
 ...
 for(let depth=1;depth<=maxDepth;depth++){let candidate=best,value=-Infinity;try{for(const m of root){check();stats.expanded++;let q=play(s,m.l,m.i,m.t);let v=minimax(q,depth-1,pruning?value:-Infinity,Infinity);if(v>value){value=v;candidate=m;}}}catch(e){if(e.message==='deadline')break;throw e;}
